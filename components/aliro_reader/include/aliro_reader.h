...
/** @brief True while a reader exists and its polling task is running. */
bool aliro_reader_is_running(void);

/**
 * @brief Reader public key as a raw uncompressed P-256 point (65 bytes).
 *
 * This is the form the Matter Door Lock cluster's AliroReaderVerificationKey
 * attribute wants, which is why it is raw rather than the PEM used elsewhere.
 */
esp_err_t aliro_reader_get_public_key_raw(uint8_t *out, size_t *out_len);

/** @brief Reader group identifier (16 bytes), as configured. */
esp_err_t aliro_reader_get_group_identifier(uint8_t *out, size_t *out_len);

/** @brief Reader group sub-identifier (16 bytes), generated by the SDK. */
 ...
 * 65 bytes of public point -- while everything below wants X.509 PEM. Both
 * conversions live here so esp_aliro_utils stays behind one component.
 *
 * @param[inout] pem_len Capacity on input, PEM length (without a terminator)
 *                       on output
 */
